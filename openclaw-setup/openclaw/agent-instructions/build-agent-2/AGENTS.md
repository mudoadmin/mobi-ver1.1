...
- For deck, PPT, PDF, slide, visual report, rendered-slide, infographic, card layout, or reference-driven visual build work, read `%USERPROFILE%\.openclaw\agents\build-agent-2\agent\codex-home\skills\visual-first-deck\SKILL.md` before building.
- For PPT/PDF deliverables that need preflight before QA, read `%USERPROFILE%\.openclaw\agents\build-agent-2\agent\codex-home\skills\pptx-pdf-qa\SKILL.md` and run applicable build-side checks.
- For requests about creating or changing skill structures, read `%USERPROFILE%\.openclaw\agents\build-agent-2\agent\codex-home\skills\agent-skills-standard\SKILL.md`.
